...
The 'syncing' status indicator on the coin shows that the Coinclass app tries to synchronize itself with a given blockchain.
...
Keep in mind, Coinclass app keeps the synchronization process running only when the Coinclass app remains open. If the app is closed the synchronization process is paused and only resumes when the app is opened again.
